...
export const Replay: React.FC<{}> = () => {
  ...
  useEffect(() => {
    if (startTime && endTime && (isEmpty(rtmMessage) || isNull(rtmMessage.count) && isEmpty(rtmMessage.messages)) && loadRTM.current === false) {
  ...
  const PlayerCover = useCallback(() => {
    if (!player || (isEmpty(rtmMessage) || isNull(rtmMessage.count) && isEmpty(rtmMessage.messages))) {
